...
# # .xls 파일을 .xlsx 파일로 변환
# pyexcel.save_as(file_name=fx_name, dest_file_name=fx_name + "x")
...
for i in range(count - 1):
    ...
    img = Image(img_path)
    img_cell = "F" + str(i + 2)  # 이미지가 삽입될 셀 위치
    ws.column_dimensions[img_cell[0]].width = img.width  # 열의 너비 조정
    ws.row_dimensions[int(img_cell[1:])].height = img.height  # 행의 높이 조정
